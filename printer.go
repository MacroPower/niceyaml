...
func (p *Printer) getTokenStringWithRanges(tokens token.Tokens) string {
	...
	// Initialize position tracking from the first token, matching the Finder's approach.
	// Position.Column points to where Value starts; subtract byte offset to find Origin start.
	firstTk := tokens[0]
	currentLine := firstTk.Position.Line
	currentCol := firstTk.Position.Column - tokenValueOffset(firstTk)
	if currentCol < 1 {
		currentCol = 1
	}
	...
	for _, tk := range tokens {
		...
		for lineIdx, line := range lines {
			...
			if !firstContentLineProcessed && valueOffset > 0 {
				// ValueOffset is the byte index within this content line where Value starts.
				// Check if the leading part is all whitespace.
				if valueOffset <= len(line) {
					sepStr := line[:valueOffset]
					if strings.TrimLeft(sepStr, " \t") == "" {
						separatorRunesInLine = len([]rune(sepStr))
					}
				}
			}
			...
			if separatorRunesInLine > 0 && separatorRunesInLine <= len(lineRunes) {
				...
				sb.WriteString(p.styleLineWithRanges(sepPart, currentLine, lineStartCol, p.colorScheme.Default))

				currentCol += separatorRunesInLine
				lineRunes = lineRunes[separatorRunesInLine:]
			}

			// Part 2: Render content portion (token style).
			if len(lineRunes) > 0 {
				contentStartCol := currentCol
				sb.WriteString(p.styleLineWithRanges(string(lineRunes), currentLine, contentStartCol, tokenStyle))

				currentCol += len(lineRunes)
			}
		}
	}
