...
func (f *Finder) buildSourceAndPositionMap(tokens token.Tokens) (string, *positionMap) {
	...
	// Position.Column points to where Value starts, but Origin may have leading whitespace.
	firstTk := tokens[0]
	line := firstTk.Position.Line
	col := firstTk.Position.Column - tokenValueOffset(firstTk)
	if col < 1 {
		col = 1
	}
	...
	for _, tk := range tokens {
		...
		for i := 0; i < len(origin); {
			r, size := utf8.DecodeRuneInString(origin[i:])

			// Record position for this character using character index.
			pm.add(charIndex, Position{Line: line, Col: col})

			charIndex++

			// Write all bytes of the rune to the string builder.
			sb.WriteString(origin[i : i+size])

			// Update position after processing entire rune.
			if r == '\n' {
				line++
				col = 1
			} else {
