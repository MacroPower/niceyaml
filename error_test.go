...
func TestErrorAnnotation(t *testing.T) {
	...
	tcs := map[string]struct {
		...
		"root path": {
			source:       "key: value",
			pathBuilder:  func() *yaml.Path { return niceyaml.NewPathBuilder().Build() },
			errMsg:       "root error",
...
- third`
	...
	err := niceyaml.NewError(
		...
		niceyaml.WithPath(niceyaml.NewPathBuilder().Index(1).Build()),
		niceyaml.WithTokens(tokens),
	)
...
another: line`
	...
	err := niceyaml.NewError(
		...
		niceyaml.WithPath(niceyaml.NewPathBuilder().Build()),
		niceyaml.WithTokens(tokens),
	)
